...
A separate cap bounds per-batch acceptance work:

- **`maxDepositsPerBatch`** — hard cap on the total deposit count across a batch's block headers, enforced in `acceptNextBatch`
