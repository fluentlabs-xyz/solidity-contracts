...
contract AcceptBatchParityTest is BaseDualFork {
    ...
    function test_bulkAcceptBatchAndProcessMessages_withProofs() public {
        ...
        SentMessageData[] memory l2ToL1 = _sendBulkL2ToL1(
            MESSAGE_COUNT,
            TRANSFER_AMOUNT
        );
        ...
        assertEq(
            l1.rollup.acceptedBatchHash(1),
            expectedRootA,
            "accepted root mismatch for batch A"
        );
        ...
        assertEq(
            l1.rollup.acceptedBatchHash(2),
            expectedRootB,
            "accepted root mismatch for batch B"
        );
        assertEq(
            l1.rollup.nextBatchIndex(),
            3,
            "nextBatchIndex must move after two withdrawal batches"
        );
        ...
        address l1PeggedTokenAddress = l1.gateway.computePeggedTokenAddress(
            address(l2.originToken)
        );
        ...
        SentMessageData[] memory l1ToL2 = _sendBulkL1ToL2WithPeggedToken(
            l1PeggedTokenAddress,
            MESSAGE_COUNT,
            TRANSFER_AMOUNT
        );
        assertEq(
            l1.bridge.getQueueSize(),
            MESSAGE_COUNT,
            "L1 queue must hold one deposit per outbound message"
        );
        ...
        (
            Rollup.BlockCommitment[] memory depositBatchA,
            Rollup.BlockCommitment[] memory depositBatchB
        ) = _buildDepositBatchesFromMessages(l1ToL2, lastHashAfterWithdrawals);
        (
            Rollup.DepositsInBlock[] memory depositsA,
            Rollup.DepositsInBlock[] memory depositsB
        ...
        assertEq(
            ...
            "queue should shrink after first deposit batch"
        );
    ...
    function test_bulkPath_preservesQueueAndConsumesDepositsCorrectly() public {
        ...
        _switchToL1();
        _assertOnL1();
        MockERC20Token l1OriginToken = new MockERC20Token(
            "L1 Origin Token",
            "L1T",
            MESSAGE_COUNT * TRANSFER_AMOUNT,
            USER_B
        ...
        for (uint256 i = 0; i < MESSAGE_COUNT; i++) {
            vm.recordLogs();
            l1.gateway.sendTokens(
                address(l1OriginToken),
                USER_A,
                TRANSFER_AMOUNT
            );
        ...
        assertEq(
            l1.bridge.getQueueSize(),
            MESSAGE_COUNT,
            "queue should contain all outbound L1 messages"
        );
        ...
        assertEq(
            l1.rollup.acceptedBatchHash(1),
            expectedRootA,
            "accepted root mismatch for first deposit batch"
        );
        assertEq(
            l1.bridge.getQueueSize(),
            MESSAGE_COUNT - BATCH_SIZE,
            "queue transition after first batch is wrong"
        );
    ...
    function _buildWithdrawalBatchesFromMessages(
        ...
        pure
        returns (
            Rollup.BlockCommitment[] memory batchA,
            Rollup.BlockCommitment[] memory batchB,
            bytes32 lastHash
        )
    {
        ...
        for (uint256 i = 0; i < MESSAGE_COUNT; i++) {
            ...
            Rollup.BlockCommitment memory c = _buildCommitment(
                prevHash,
                blockHash,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < MESSAGE_COUNT; i++) {
            ...
            bytes32 depositHash = keccak256(
                abi.encodePacked(messages[i].messageHash)
            );
            Rollup.BlockCommitment memory c = _buildCommitment(
                prevHash,
                blockHash,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < batchA.length; i++) {
            depositsA[i] = Rollup.DepositsInBlock({
                blockHash: batchA[i].blockHash,
                depositCount: 1
            });
            depositsB[i] = Rollup.DepositsInBlock({
                blockHash: batchB[i].blockHash,
                depositCount: 1
            });
        }
    }
    ...
    function _proveWithdrawalBatch(
        ...
        for (uint256 i = 0; i < batch.length; i++) {
            MerkleTree.MerkleProof memory blockProof = _buildBlockProof(
                batch,
                i
            );
