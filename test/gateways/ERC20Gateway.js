...
describe("ERC20Gateway", function () {
    ...
    before(async function () {
        ...
        const { gateway } = await deployERC20GatewayProxy(ethers, accounts[0].address, bridge.target, tokenFactory.target);
        ...
        await erc20Gateway.setOtherSide(
    ...
    it("Send tokens test", async function () {
        ...
        const send_tx = await contractWithSigner.sendTokens(token.target, accounts[3].address, 100);
    ...
    it("Receive tokens test", async function () {
        ...
        const peggedTokenAddress = await tokenFactory.computePeggedTokenAddress(erc20Gateway.target, token.target);
        ...
        try {
            ...
            expect(error.toString()).to.equal(
                "Error: VM Exception while processing transaction: " + "reverted with custom error 'MessageReceivedOutOfOrder()'"
            );
        }
    });
});
