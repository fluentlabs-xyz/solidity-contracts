...
import {
    IL1BlockOracle
} from "../../contracts/interfaces/oracles/IL1BlockOracle.sol";
...
contract L1BlockOracleTest is Test {
    ...
    function test_updateL1BlockNumber_isMonotonic() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IL1BlockOracle.BlockNotMonotonic.selector,
                uint256(100),
                uint256(99)
            )
        );
    ...
    function test_RevertIf_updateL1BlockNumber_callerNotSubmitter() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                IL1BlockOracle.UnauthorizedSubmitter.selector,
                user
            )
        );
        vm.prank(user);
    ...
    function test_setL1BlockNumber_ownerCanSetBackwards() public {
        ...
        assertEq(
            oracle.getL1BlockNumber(),
            50,
            "owner should bypass monotonicity"
        );
    }
    ...
    function test_RevertIf_setL1BlockNumber_zero() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                IL1BlockOracle.L1BlockNumberZeroNotAllowed.selector
            )
        );
    ...
    function test_RevertIf_setL1BlockNumber_exceedsMax() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IL1BlockOracle.L1BlockNumberTooLarge.selector,
                maxBn + 1,
                maxBn
            )
        );
    ...
    function test_RevertIf_setSubmitter_zeroAddress() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                IL1BlockOracle.ZeroAddressNotAllowed.selector,
                "submitter"
            )
        );
