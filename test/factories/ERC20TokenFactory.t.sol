...
contract ERC20TokenFactoryTest is FactoryTestBase {
    ...
    function testComputePeggedTokenAddressMatchesDeployment() public {
        address predicted = factory.computePeggedTokenAddress(abi.encode(GATEWAY, ORIGIN_TOKEN), "");
    ...
    function testSharedComputePeggedTokenAddressInterfaceMatchesComputePeggedTokenAddress() public view {
        bytes memory keyData = abi.encode(GATEWAY, ORIGIN_TOKEN);
        address fromComputePeggedTokenAddress = factory.computePeggedTokenAddress(keyData, "");
        address fromSharedPeggedAddress = factory.computePeggedTokenAddress(keyData, "");

        assertEq(fromSharedPeggedAddress, fromComputePeggedTokenAddress, "shared pegged address interface mismatch");
    }

    function testDeployTokenThroughGenericInterfaceWorks() public {
        bytes memory keyData = abi.encode(GATEWAY, ORIGIN_TOKEN);
        address predicted = factory.computePeggedTokenAddress(keyData, "");
    ...
    function testDeployTokenRevertsOnInvalidGateway() public {
        vm.expectRevert(abi.encodeWithSelector(IGenericTokenFactoryErrors.ZeroAddressNotAllowed.selector, "Gateway"));
        _deployViaSharedInterface(address(0), ORIGIN_TOKEN);
    }

    function testDeployTokenRevertsOnInvalidOriginToken() public {
        vm.expectRevert(abi.encodeWithSelector(IGenericTokenFactoryErrors.ZeroAddressNotAllowed.selector, "OriginToken"));
        _deployViaSharedInterface(GATEWAY, address(0));
    }

    function testOnlyPaymentGatewayOrOwnerCanDeploy() public {
        bytes memory keyData = abi.encode(GATEWAY, ORIGIN_TOKEN);
        vm.prank(ATTACKER);
    ...
    function testOnlyPaymentGatewayOrOwnerCanDeployThroughGenericInterface() public {
        bytes memory keyData = abi.encode(GATEWAY, ORIGIN_TOKEN);
        vm.prank(ATTACKER);
        vm.expectRevert(IGenericTokenFactoryErrors.OnlyPaymentGatewayOrOwner.selector);
        IGenericTokenFactory(address(factory)).deployToken(keyData, "");
    ...
    function testComputeOtherSidePeggedTokenAddressMatchesManualCompute() public view {
        bytes memory keyData = abi.encode(GATEWAY, ORIGIN_TOKEN);
        address computed = factory.computeOtherSidePeggedTokenAddress(keyData, "");
        address local = factory.computePeggedTokenAddress(keyData, "");
