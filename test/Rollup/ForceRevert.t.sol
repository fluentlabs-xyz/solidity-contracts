...
contract ForceRevertTest is RollupAssertions {
    ...
    function test_forceRevert_restoresDepositsToQueueInOriginalOrder() public {
        ...
        assertEq(depositBridge.getSentMessageQueueSize(), 0, "both deposits consumed by acceptNextBatch");
    ...
    function test_acceptNextBatch_snapshotsBridgeCursorIntoBatchRecord() public {
        // Batch 1: no deposits
        ...
        bytes32 lastHash = rollup.lastBlockHashInBatch(batch1);
        L2BlockHeader[] memory batch2Headers = _makeBatchWithDeposits3(lastHash, 0, d1, d2, d3);
        vm.prank(sequencer);
        rollup.acceptNextBatch(batch2Headers, 1);
        uint256 batch2 = batch1 + 1;
    ...
    function test_forceRevert_multipleBatches_rewindsCursorToOldestSnapshot() public {
        ...
        L2BlockHeader[] memory batch2Headers = _makeBatchWithDeposits(lastHash1, 0, a, b);
        vm.prank(sequencer);
        rollup.acceptNextBatch(batch2Headers, 1);
        uint256 batch2 = batch1 + 1;
        _submitBlobs(batch2, 0);
        ...
        bytes32 lastHash2 = rollup.lastBlockHashInBatch(batch2);
        L2BlockHeader[] memory batch3Headers = _makeBatchWithDeposits1(lastHash2, 0, c);
        vm.prank(sequencer);
        rollup.acceptNextBatch(batch3Headers, 1);
    ...
    /// @dev forceRevertBatch with toBatchIndex == lastAcceptedBatchIndex is a no-op for
    ...
    /// @dev Builds a batch whose `headerIndex` header commits to a single deposit.
    ...
    /// @dev Builds a batch whose `headerIndex` header commits to three deposits.
    function _makeBatchWithDeposits3(
        ...
        ids[2] = d3;
        batch[headerIndex].depositRoot = keccak256(abi.encodePacked(ids));
        batch[headerIndex].depositCount = 3;
    }
}
