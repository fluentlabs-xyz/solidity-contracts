...
import {L2BlockHeader, BlockDeposit, InitConfiguration} from "../../contracts/interfaces/rollup/IRollupTypes.sol";
...
contract CorruptionTest is RollupAssertions {
    ...
    function test_corrupt_daDeadlineExceeded() public {
        ...
        vm.prank(sequencer);
        r.commitBatch(_computeBatchRoot(batch), GENESIS_HASH, batch[batch.length - 1].blockHash, uint24(batch.length), new BlockDeposit[](0), 1);
    ...
    function test_corrupt_checksOldestNonFinalizedBatch() public {
        ...
        // Accept batch 2 — Committed, submit blobs window will expire
        uint256 batchIndex = rollup.nextBatchIndex();
        L2BlockHeader[] memory headers = _makeBatch(lastHash);
        vm.prank(sequencer);
        rollup.commitBatch(_computeBatchRoot(headers), lastHash, headers[headers.length - 1].blockHash, uint24(headers.length), new BlockDeposit[](0), 1);
        ...
        bytes32 lastHash2 = headers[headers.length - 1].blockHash;
        L2BlockHeader[] memory headers3 = _makeBatch(lastHash2);
        vm.prank(sequencer);
        rollup.commitBatch(_computeBatchRoot(headers3), lastHash2, headers3[headers3.length - 1].blockHash, uint24(headers3.length), new BlockDeposit[](0), 1);
    ...
    function test_corruptedBatchBlocksNewAcceptance() public {
        ...
        vm.prank(sequencer);
        rollup.commitBatch(_computeBatchRoot(batch), lastHash, batch[batch.length - 1].blockHash, uint24(batch.length), new BlockDeposit[](0), 1);
    }
    ...
    function test_challengeBlock_revertsWhenCorrupted() public {
        ...
        vm.prank(sequencer);
        rollup.commitBatch(_computeBatchRoot(batch2Commits), lastHash1, batch2Commits[batch2Commits.length - 1].blockHash, uint24(batch2Commits.length), new BlockDeposit[](0), 1);
    ...
    function test_healthy_preconfirmDeadlineAtBoundary() public {
        ...
        vm.roll(block.number + PRECONFIRM_WINDOW);
        assertFalse(rollup.isRollupCorrupted());
    }

    function test_healthy_preconfirmedBeforeDeadline() public {
        uint256 batchIndex = _acceptBatch(GENESIS_HASH, 0);
        _submitBlobs(batchIndex, 0);
    ...
    function _deployRollupWithConfig(uint256 daDeadline) private returns (Rollup) {
        ...
        cfg.preconfirmWindow = daDeadline + 3700;
        cfg.challengeWindow = daDeadline + 7400;
        cfg.finalizationDelay = daDeadline + 14700;
