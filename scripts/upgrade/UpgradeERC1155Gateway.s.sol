...
/// @notice Upgrades ERC1155Gateway proxy to the latest implementation.
/// @dev Env: PROXY_ADDRESS (required), UNSAFE_SKIP_STORAGE_CHECK (optional, for first upgrade without reference build).
contract UpgradeERC1155Gateway is Script {
    function run() external {
        address proxy = vm.envAddress("PROXY_ADDRESS");
        require(proxy.code.length > 0, "proxy has no code");

        Options memory opts;
        opts.referenceContract = vm.envOr("REFERENCE_CONTRACT", string("ERC1155Gateway.sol:ERC1155Gateway"));
        opts.unsafeSkipStorageCheck = vm.envOr("UNSAFE_SKIP_STORAGE_CHECK", true);

        vm.startBroadcast();
        Upgrades.upgradeProxy(proxy, "ERC1155Gateway.sol:ERC1155Gateway", "", opts);
        vm.stopBroadcast();
