...
/// @notice Upgrades ERC721TokenFactory proxy to the latest implementation.
/// @dev Env: PROXY_ADDRESS (required). Uses safe Upgrades API with storage layout validation.
contract UpgradeERC721TokenFactory is Script {
    function run() external {
        ...
        vm.startBroadcast();
        Upgrades.upgradeProxy(proxy, "ERC721TokenFactory.sol:ERC721TokenFactory", "", opts);
        vm.stopBroadcast();
