...
/// @notice Upgrades NativeGateway proxy to the latest implementation.
/// @dev Env: PROXY_ADDRESS (required). Uses safe Upgrades API with storage layout validation.
contract UpgradeNativeGateway is Script {
    function run() external {
        address proxy = vm.envAddress("PROXY_ADDRESS");
        require(proxy.code.length > 0, "proxy has no code");

        vm.startBroadcast();
        Upgrades.upgradeProxy(proxy, "NativeGateway.sol:NativeGateway", "");
        vm.stopBroadcast();
