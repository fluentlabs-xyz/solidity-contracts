...
/// @notice Upgrades ERC1155TokenFactory proxy to the latest implementation.
/// @dev Env: PROXY_ADDRESS (required). Uses safe Upgrades API with storage layout validation.
contract UpgradeERC1155TokenFactory is Script {
    function run() external {
        ...
        vm.startBroadcast();
        Upgrades.upgradeProxy(proxy, "ERC1155TokenFactory.sol:ERC1155TokenFactory", "", opts);
        vm.stopBroadcast();
