...
library DrandQuicknetVerifier {
    ...
    /// drand quicknet's RFC 9380 domain separation tag, 43 bytes — scheme
    /// `bls-unchained-g1-rfc9380`, chain
    /// `52db9ba70e0cc0f6eaf7803dd07447a1f5477735fd3f661792ba94600c84e971`.
    bytes private constant DST =
        hex"424c535f5349475f424c53313233383147315f584d443a5348412d3235365f535357555f524f5f4e554c5f";
    ...
    function verify(uint64 round, bytes calldata sigUncompressed) internal view returns (bool) {
        ...
        bytes memory h = _hashToG1(abi.encodePacked(sha256(abi.encodePacked(round))));
        _rejectInfinity(h);

        // 2-pair input: (sig‖-G2gen) ‖ (H‖pk) = 768 bytes.
        // Equation: e(sig, -G2gen) · e(H, pk) == 1.
        bytes memory input = bytes.concat(sigUncompressed, NEG_G2_GENERATOR, h, PUBLIC_KEY);
    ...
    function _hashToG1(bytes memory input) internal view returns (bytes memory) {
        // DST' = dst ‖ I2OSP(len(dst), 1). quicknet's DST is 43 bytes, inside
        // RFC 9380's short-DST path, so the §5.3.3 oversize workaround is absent.
    ...
    /// uniform[off:off+64]
    function _slice64(bytes memory uniform, uint256 off) internal pure returns (bytes memory r) {
        r = new bytes(64);
    ...
    function _modexpModP(bytes memory base64) internal view returns (bytes memory) {
        bytes memory input =
            bytes.concat(bytes32(uint256(64)), bytes32(uint256(1)), bytes32(uint256(48)), base64, bytes1(uint8(1)), P);
    ...
    ///      false → nothing is published).
    function _mapFpToG1(bytes memory fp48) internal view returns (bytes memory) {
    ...
    function _g1Add(bytes memory p0, bytes memory p1) internal view returns (bytes memory) {
    ...
    function _slice48(bytes memory src, uint256 off) internal pure returns (bytes memory r) {
        r = new bytes(48);
    ...
    function _fpIsZero(bytes memory fp48) internal pure returns (bool) {
        for (uint256 i = 0; i < 48; i++) {
